...
def insert_gbq(data, table, gbq_schema):
	...
	data = data.applymap(lambda x: x.replace(u'\r', u' ').replace(u'\n', u' ') if isinstance(x, str) or isinstance(x, unicode) else x)
	...
	data.to_gbq(gbq_schema + '.' + table, config.goog_project, chunksize=config.chunk, if_exists='append', private_key=config.goog_private_key)
