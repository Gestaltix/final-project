...
def make_df(file_path, file_type):
    ...
    if file_type in ('fieldwiz'):
        ...
        df[u'Acceleration'] = (df[u'Velocity'] - df[u'Velocity'].shift(1)) / pd.to_timedelta(
            df[u'ts_seconds'] - df[u'ts_seconds'].shift(1), unit='s').dt.total_seconds()
        df.set_index("ts_seconds", inplace=True)

        df.drop(
            [u'time[ISO-UTC]', u'xPos[m]', u'yPos[m]', u'hr[bpm]', u'speed[km/h]', u'Latitude[deg]', u'Longitude[deg]',
             u'numSatFix[-]', u'hdop[-]', u'speed[km/h]'], axis=1, inplace=True)
        ...
        df.rename(columns={'speed': u'Velocity', 'acceleration': u'Acceleration', 'ts_utc': u'ts_seconds'},
                  inplace=True)
